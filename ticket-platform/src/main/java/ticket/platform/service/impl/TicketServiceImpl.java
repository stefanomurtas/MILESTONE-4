...
public class TicketServiceImpl implements TicketService {
    ...
    public List<TicketDto> getAll() {
        return ticketRepository.findAll().stream()
                .map(item -> new TicketDto(item.getId(), item.getState(), item.getTitle(), item.getDetails()))
                .collect(Collectors.toList());
    }

    @Override
    public List<TicketDto> getAllByCategory(String category) {
    ...
    public List<TicketDto> getAllByStatus(TicketStatus ticketStatus) {
        return ticketRepository.findAllByState(ticketStatus).stream()
                .map(item -> new TicketDto(item.getId(), item.getState(), item.getTitle(), item.getDetails()))
                .collect(Collectors.toList());
    }
